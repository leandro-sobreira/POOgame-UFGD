...
# This ensures it's stored alongside the source code.
DB_FILE = os.path.join(os.path.dirname(__file__), 'gamedata.dat')

def load_data():
    """
    Loads data from the pickle file. If the file doesn't exist,
...
def get_player(name):
    ...
    # From our code, a new Blackjack player starts with 1000 points.
    new_player = {
        "name": name,
...
def get_ranking(game_key, limit=5):
    """
    Returns a sorted list of players for the ranking screen.
    'game_key' must be a key in the player dictionary, e.g., 'blackjack_points'.
    """
    data = load_data()
    # Sort the players list in descending order based on the provided game_key.
    sorted_players = sorted(data["players"], key=lambda p: p.get(game_key, 0), reverse=True)
    
    return sorted_players[:limit]  # Return only the top players, e.g., top 5.
